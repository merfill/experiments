...
def transform_sentence(sequence, dictionary):
    return dictionary.get_ids(sequence.split(' '))


def create_example_train(row, dictionary):
    """
    Creates a training example for the Ubuntu Dialog Corpus dataset.
    Returnsthe a tensorflow.Example Protocol Buffer object.
    """
    context, utterance, label = row
    context_transformed = transform_sentence(context, dictionary)
    utterance_transformed = transform_sentence(utterance, dictionary)
    context_len = len(context_transformed)
    utterance_len = len(utterance_transformed)
...
def create_example_test(row, dictionary):
    """
...
if __name__ == "__main__":
    ...
    create_tfrecords_file(
            ...
            example_fn=functools.partial(create_example_test, dictionary=d))
    ...
    create_tfrecords_file(
            ...
            example_fn=functools.partial(create_example_train, dictionary=d))

    print('Total vocabulary size: {}'.format(d.get_dict_size()))
    dict_path = os.path.join(FLAGS.output_dir, 'dictionary.json')
    print("Creating dictionary file at {}...".format(dict_path))
    d.save(dict_path)
